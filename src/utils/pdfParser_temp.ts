...
export function parseBillText(text: string): Record<string, string> {
  ...
  const isGrupoB = (modalidadeTarifaria && modalidadeTarifaria.toUpperCase().includes("CONVENCIONAL")) || (subgrupo && subgrupo.toUpperCase().includes("B3"));
  ...
  let demandaContratadaPonta = extract(/(\d+(?:,\d+)?)\s*Demanda\s+ponta/i) || extract(/Demanda\s+ponta.*?[\D](\d+(?:,\d+)?)/i) || "0";
  let demandaContratadaForaPonta = extract(/(\d+(?:,\d+)?)\s*Demanda\s+fora\s+ponta/i) || extract(/Demanda\s+fora\s+ponta.*?[\D](\d+(?:,\d+)?)/i) || "0";
  const getExplicitMedida = (type) => {
  ...
  let demandaPotenciaMedidaPonta = getExplicitMedida('ponta') || extract(/Demanda Medida.*Ponta[^\d]*([\d\.,]+)/i) || "0";
  let demandaPotenciaMedidaForaPonta = getExplicitMedida('fora ponta') || extract(/Demanda Medida.*Fora Ponta[^\d]*([\d\.,]+)/i) || "0";
  ...
  let energiaReativaExcedPonta = "DEBUG: " + demandaLines.join(" | ").substring(0, 500);
  ...
  if (concessionaria === "ENERGISA" && !isGrupoB) {
    ...
    const allTariffMatches = Array.from(text.matchAll(/\d{1,3}[\.,]\d{6}/g)).map(m => ({ val: parseFloat(m[0].replace(/[^\d]/g, "")) / 1000000, pos: m.index || 0 }));
    ...
    if (tariffMatches.length > 0) {
      ...
      let qForaPontaRegexStr = extract(/Fora Ponta Energia ativa em kWh.*?(?<![\d\,])(\d{1,5}(?:\.\d{3})*,\d+)/i) || extract(/Consumo em kWh - Fora Ponta.*?(?<![\d\,])(\d{1,5}(?:\.\d{3})*,\d+)/i);
      ...
      // é porque os valores são muito pequenos ou concatenados. N.o usar fallback neste caso.
  ...
  if (concessionaria === "ENERGISA") {
    ...
    const tariffMatches = Array.from(text.matchAll(/\d{1,3}[\.,]\d{6}/g)).map(m => parseFloat(m[0].replace(/[^\d]/g, "")) / 1000000);
    ...
    const getCandidates = (contratada: number) => {
        let candidates = [];
        let tolerance = Math.max(3.0, contratada * 0.1);
        for (let p of demandaPairs) {
            if (Math.abs(p.q - contratada) < tolerance) candidates.push({ type: 'single', p1: p, p2: null });
        }
    ...
    if (isVerde && demandaPairs.length > 0) {
        // VERDE: handle demand with possible "N.o Consumida" or "Ultrapassagem" split
        let textHasNcVerde = /N.o\s+Consumida/i.test(text);
        ...
        if (contForaPonta > 0) {
            if (textHasNcVerde) {
                ...
                if (bestNcPair) {
                    if (demandaPotenciaMedidaForaPonta === "0" || demandaPotenciaMedidaForaPonta === "0,00") {
                    ...
                    if (singleNcMatch && singleNcMatch.q < contForaPonta) {
                        ...
                        if (textHasMedidaFP && (demandaPotenciaMedidaForaPonta === "0" || demandaPotenciaMedidaForaPonta === "0,00")) {
        ...
        } else if (candPonta.length === 0 && candFora.length >= 2) {
            let sorted = candFora.sort((a,b) => Math.abs(a.p1.q - contForaPonta) - Math.abs(b.p1.q - contForaPonta));
            let cf1 = sorted[0];
            let cf2 = sorted[1];
            if (cf1.p1.t > cf2.p1.t) {
                validCombinations.push({ cp: cf1, cf: cf2 });
            } else {
                validCombinations.push({ cp: cf2, cf: cf1 });
            }
        } else if (candPonta.length >= 2 && candFora.length === 0) {
            let sorted = candPonta.sort((a,b) => Math.abs(a.p1.q - contPonta) - Math.abs(b.p1.q - contPonta));
            let cp1 = sorted[0];
            let cp2 = sorted[1];
            if (cp1.p1.t > cp2.p1.t) {
                validCombinations.push({ cp: cp1, cf: cp2 });
            } else {
                validCombinations.push({ cp: cp2, cf: cp1 });
            }
        ...
        // Check if the PDF text explicitly mentions "N.o Consumida" for Ponta and/or Fora Ponta
        let textHasNcPonta = /N.o\s+Consumida\s*-\s*Ponta/i.test(text);
        let textHasNcForaPonta = /N.o\s+Consumida\s*-\s*F(?:ora)?\s*Ponta/i.test(text);
        ...
        if (validCombinations.length > 0) {
            ...
            if (best.cp) {
                ...
                if (demandaPotenciaMedidaPonta === "0" || demandaPotenciaMedidaPonta === "0,00") {
  ...
  if (concessionaria === "ENERGISA" && text.includes("GDI")) {
    ...
    if (oUcCount > 0) {
      const targetCount = mUcCount === 0 ? gdiValuesR$.length : oUcCount;
  ...
  currentIndex = 0;
  if (mUcCount > 0) {
    const targetCount = oUcCount === 0 ? gdiValuesR$.length : mUcCount;
    for (let i = 0; i < targetCount; i++) {
      if (currentIndex < gdiValuesR$.length) {
        const rVal = gdiValuesR$[currentIndex];
  ...
  return {
    ...
    energiaAtvInjetadaGDIMUC: formatNumberToBR(energiaAtvInjetadaGDIMUC),])\d{1,5}(?:[\.,]\d{3})*[\.,]\d{2}(?![\d\.,])/g)).map(m => parseFloat(m[0].replace(/[^\d]/g, "")) / 100).join(" | ").substring(0, 1500),
